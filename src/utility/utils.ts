...
export const generateCells = (): Cell[][] => {
  ...
  for (let row = 0; row < MAX_ROWS; row++) {
    ...
    for (let col = 0; col < MAX_COLS; col++) {
      cells[row].push({
        value: CellValue.none, //Bomb, None, one, two, ...etc
  ...
  for (let rowIndex = 0; rowIndex < MAX_ROWS; rowIndex++) {
    for (let colIndex = 0; colIndex < MAX_COLS; colIndex++) {
      ...
      //Checks if there is a bomb near the cell
      if (topLeftCell?.value === CellValue.bomb) numOfBombs++;
      if (topCell?.value === CellValue.bomb) numOfBombs++;
      if (topRightCell?.value === CellValue.bomb) numOfBombs++;
      if (leftCell?.value === CellValue.bomb) numOfBombs++;
      if (rightCell?.value === CellValue.bomb) numOfBombs++;
      if (bottomLeftCell?.value === CellValue.bomb) numOfBombs++;
      if (bottomCell?.value === CellValue.bomb) numOfBombs++;
      if (bottomRightCell?.value === CellValue.bomb) numOfBombs++;

      //Shows the value of the cell
      if (numOfBombs > 0) {
